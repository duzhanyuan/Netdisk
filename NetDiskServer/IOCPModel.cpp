...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	if (INVALID_SOCKET == m_pListenContext->m_Socket) 
	{
		...
		AfxMessageBox(_T("初始化Socket失败，错误代码: %d.\n", WSAGetLastError()));
		return false;
	}
	else
	{
	...
	{  
		...
		AfxMessageBox(_T("绑定 Listen Socket至完成端口失败！错误代码: %d/n", WSAGetLastError()));
	...
	if (SOCKET_ERROR == listen(m_pListenContext->m_Socket,SOMAXCONN))
	{
		...
		AfxMessageBox(_T("Listen()函数执行错误.\n"));
	...
	{  
		...
		AfxMessageBox(_T("WSAIoctl 未能获取AcceptEx函数指针。错误代码: %d\n"), WSAGetLastError());
		this->_DeInitialize();
		return false;  
	}  
	...
	{  
		...
		AfxMessageBox(_T("WSAIoctl 未能获取GuidGetAcceptExSockAddrs函数指针。错误代码: %d\n"), WSAGetLastError());
...
bool CIOCPModel::_PostAccept( PER_IO_CONTEXT* pAcceptIoContext )
{
	...
	{  
		...
		AfxMessageBox(_T("创建用于Accept的Socket失败！错误代码: %d"), WSAGetLastError());
		return false;  
	} 
	...
	{  
		...
		{  
			...
			AfxMessageBox(_T("投递 AcceptEx 请求失败，错误代码: %d"), WSAGetLastError());
...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	CString userInfo;
	userInfo=pIoContext->m_wsaBuf.buf;
	int ret=UserLogin(userInfo);
	
	//////////////////////////////////////////////////////////////////////////
	//返回用户登录信息
	char szTemp[MAX_BUFFER_LEN];
	sprintf(szTemp,"%d",ret);
	int retvalue=send(pIoContext->m_sockAccept,szTemp,sizeof(szTemp),0);
	...
	{
		/*CString msg;
		msg.Format(_T("客户端 %s:%d 发送信息:%s"),inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port),pIoContext->m_wsaBuf.buf);
		AfxMessageBox(msg);*/
		//////////////////////////////////////////////////////////////////////////////////////////////////////
...
//判断接入连接的用户是否存在
int CIOCPModel::UserLogin(CString Userinfo)
{
