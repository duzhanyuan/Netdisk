...
BOOL IOCPS::Startup()
{
	...
	if ( m_iWSAInitResult != NO_ERROR )
	{
		AppendLog(_T("WSAStartup失败，系统不支持！"));
		return FALSE;	
	}else
	{
		AppendLog(_T("Winsock 2.0 成功加载！"));
	}
	...
	if ( !m_bServerStarted )
	{
		...
		msg.Format(_T("Number of pending asynchronous reads: %d"),m_iNumberOfPendlingReads);
		AppendLog(msg);
		...
		if ( m_bSendInOrder )
			AppendLog(_T("Send ordering initialized. (Decreases the performance by ~3%)"));

		if ( m_bReadInOrder )
			AppendLog(_T("Read ordering initialized.(Decreases the performance by ~3%)"));
		...
		if( bRet)
		{
			AppendLog(_T("Completionport successfully created."));		
		}
		// Config the Listner.. 
		if ( m_nPortNumber>0 )
		{
			...
			if( bRet )
			{
				AppendLog(_T("Connection listner thread successfully started."));		
			}
		}
		// Setup the IOWorkers.. 
		bRet&=SetupIOWorkers();
			...
			if ( events.lNetworkEvents & FD_ACCEPT)
			{
				if ( events.iErrorCode[FD_ACCEPT_BIT] == 0&&pThis->m_bAcceptConnections&&!pThis->m_bShutDown)
				{
					...
					clientSocket = WSAAccept(pThis->m_socListen,
						NULL,
						&nLen,(LPCONDITIONPROC)ConnectAcceptCondition,(DWORD)pThis); 
#else
					clientSocket = WSAAccept(pThis->m_socListen,
						NULL,
						&nLen,0,0); 
...
void IOCPS::OnPostedPackage(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	if( !pContext )
	{
		AppendLog(_T("FATAL ERROR OnPostedPackage pContext=NULL"));
		return; 
	}
...
{	
	...
	{	
		...
		{		
			...
			if(m_ContextMap[(unsigned int&)(pContext->m_Socket)]!=NULL)
			{
				bRet=m_ContextMap.RemoveKey((unsigned int&)(pContext->m_Socket));
				if(bRet)
					m_NumberOfActiveConnections--;
			}	
			m_ContextMapLock.Unlock();
...
void IOCPS::ShutDown()
{
	...
	if(m_bServerStarted)
	{
		...
		AppendLog(_T("Shutdown initialized."));

		m_bAcceptConnections=FALSE;

		AppendLog(_T("Sending shutdown signal to logical worker threads."));
		ShutDownWorkers();

		// We Let the IOWorker Take care of the last packets and die. 
		AppendLog(_T("Disconnecting all the Connections..."));
		DisconnectAll();



		AppendLog(_T("Sending shutdown signal to IO worker threads."));
		m_bShutDown=TRUE;
		ShutDownIOWorkers();


		AppendLog(_T("Closing Completion port.."));
		// Close the CompletionPort and stop any more requests (Stops the Listner) 
		CloseHandle(m_hCompletionPort);
		if(m_nPortNumber>0)
		{
			AppendLog(_T("Closing listner thread."));
			WSACloseEvent(m_hEvent);
			closesocket(m_socListen);
		}
		AppendLog(_T("Deallocate memory used for Clients. "));
		FreeClientContext();
		AppendLog(_T("Deallocate memory used for Buffers. "));
		FreeBuffers();
...
BOOL IOCPS::ARead(ClientContext *pContext,CIOCPBuffer *pOverlapBuff)
{
	...
	if(pContext->m_Socket!=INVALID_SOCKET )
	{
		...
		if(pOverlapBuff==NULL) 
		{			
		  AppendLog(_T("AllocateBuffer(IORead) == NULL."));
		  ReleaseClientContext(pContext);
		  return FALSE;
		}
...
BOOL IOCPS::AZeroByteRead(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	...
	if(pContext->m_Socket!=INVALID_SOCKET )
	{
		...
		if(pOverlapBuff==NULL) 
		{			
		  AppendLog(_T("AllocateBuffer(IOZeroByteRead) == NULL."));
		  ReleaseClientContext(pContext);
		  return FALSE;
		}
...
BOOL IOCPS::SetupIOWorkers()
{
	...
	for(int i=0; i<m_iMaxIOWorkers;i++)
	{
		...
		else
		{
			...
			CString msg;
			msg.Format(_T("Error Could not start worker: %s"),ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
...
*
*/
BOOL IOCPS::Connect(CString &strIPAddr, int nPort)
{
	...
	char* p;
	int len = strIPAddr.GetLength(); 
	p = (char*)malloc((len*2+1)*sizeof(char));
	//CString的长度中汉字算一个长度 
	memset(p, 0, 2*len+1); 
	USES_CONVERSION; 
	strcpy((LPSTR)p,OLE2A( strIPAddr.LockBuffer())); 

	SockAddr.sin_addr.s_addr = inet_addr(p);
...
BOOL IOCPS::PrepareSendFile(ClientContext *pContext, LPCTSTR lpszFilename)
{
	...
	{    // Already in send mode
		...
		if(pOverlapBuff)
		{
			...
			{
				AppendLog(_T("Error CreatePackage(1,iFileSize,sFileName) FAILED.."));
				ReleaseBuffer(pOverlapBuff);
				return FALSE;
			}
		}else
	...
	if (pContext->m_File.m_hFile != NULL)
	{
		...
		}else
		{
			...
			*/
			...
			{
				pContext->m_ContextLock.Unlock();
				AppendLog(_T("Error could not allocate buffer"));
				ReleaseBuffer(pOverlapBuff);
				DisconnectClient(pContext);
				return; 
			}
...
{
	CString sKey_PATH=_T("\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters");
	CString sKey_SynAttackProtect=_T("SynAttackProtect");
	CString sKey_TcpMaxHalfOpen=_T("TcpMaxHalfOpen");
	CString sKey_TcpMaxHalfOpenRetried=_T("TcpMaxHalfOpenRetried");
	CString sKey_TcpMaxPortsExhausted=_T("TcpMaxPortsExhausted");
	CString sKey_TcpMaxConnectResponseRetransmissions=_T("TcpMaxConnectResponseRetransmissions");
